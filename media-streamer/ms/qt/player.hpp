...
private slots:
    ...
    void on_representation_combo_box_current_index_changed(int index);
...
private:
    ...
    std::string get_url();
    const dash::mpd::IPeriod *get_period(std::shared_ptr<const dash::mpd::IMPD> mpd);
    ...
    std::optional<media::manager::configuration> make_configuration(std::shared_ptr<const dash::mpd::IMPD> mpd);
    void update_configuration(std::shared_ptr<const dash::mpd::IMPD> mpd);
